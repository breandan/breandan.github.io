---
layout: post
title: Sampling without replacement in 𝒪(1) space and time
...
At first glance, uniform sampling might not seem all that useful as there are more intelligent algorithms if we know something about the inputs. The key property we want from a random sampler is that it explores all regions as early possible and produces a steady stream of results, not all clumped together in some places and bad in others. If there are clumpy spots of good or bad guesses, our stream will contain them roughly proportional to their density. Wherever the data distribution lies, we will find it eventually. Later we can think about adaptive sampling, but first we must *explore*.

How do we sample a random order over all guesses? We could use a Fisher-Yates or Knuth shuffle. But that requires storing the entire search space in memory, so we're back to square one. What if we drew a random integer representing the bit pattern of the object and used that to constructed our guess directly?
...
This solves the issue of duplicates and runs in nearly constant time at first, but consumes an increasing about of time and space the longer it runs. While there are probabilistic datastructures like Bloom filters to mitigate spatial growth and provide high-accuracy membership queries, these do not address the issue of rejection sampling. Let go through our criteria again. For medium- to large-cardinality sets which are still tractable to exhaustively search, we would like a procedure that:

```
1.) Eventually visits all elements in the search space
2.) Produces samples in a [pseudo]random order
3.) Does not repeat any samples before halting
4.) Draws positive/negative samples with wall-clock frequency 
    proportional to their true density across the search space
5.) Consumes constant time and space per sample
6.) Determinstical halting after fixed time elapsed
7.) Reproducible given the same initial seed
8.) Scales to arbitrarily large search spaces
```

It turns out all of the above criteria are simultaneously satisfiable. We will now describe an elegant procedure for doing so.
...
The skeptical reader might note that since we are using a matrix, the time and space complexity is not constant. However, we note that the matrix is sparse, and computing the matrix-vector product can be reduced to exactly six bitwise operations. It can be visualized as the following circuit:
...
By selecting the coefficients `a`..`e` carefully, we can ensure the period will be full. For example, if we use the coefficients above, i.e. `[1 0 1 0 1]`,  the state vector will not repeat itself for `31` time steps, i.e., the cardinality of the underlying set. Be assured this fact is no mere coincidence.
...
Galois fields are fields with a finite number of elements. They have lots of applications in number theory, cryptography, and as it turns out, probabilistic programming. We can generate every nonzero element of the field using something called its primitive polynomial, a special function which maps elements of the field back onto themselves. There are various functions which map the field onto itself. The one whose cycle is maximal is called the primitive polynomial. It may be non-unique.

This means the primitive polynomial is a space-filling curve, i.e., a curve which visits all elements of the set exactly once before repeating. However unlike simple enumeration schemes like Hilbert Curves or Gray Codes, there is usually some kind of nontrivial bijection between the field elements and PP making it very difficult to predict where the function will take us more than a few timesteps ahead. (Although not impossible for an adversary. More sophisticated versions exist which make it resistant to adversarial attacks.) 

In particular, we can use the primitive polynomial to construct a finite state machine or linear finite state register (LFSR) whose intermediate states are all nearly indistinguishable from a true RNG on many summary statistics. This makes them useful proxies for simulating stochasticity on a deterministic machine. Their maximal period allows us to generate the entire set by cycling through its primitive polynomial, which is useful for unbiased sampling without replacement.

How do we find primitive polynomials? We could simply enumerate all polynomials up to `log_p(|S|)` and check whether they are full period. As it happens, we have already seen a guess-and-check algorithm that can generate examples in random order. This works for low order polynomials but quickly becomes intractable with increasing degree. Perhaps there is some theory we could use to help us find these polynomials.

The beauty of the primitive polynomials over GF(2) is that they can be composed using other primitive polynomials. This fact allows us to quickly eliminate certain guesses using a static property of the polynomial rather than measuring its periodicity by simulating the dynamics, which might not complete before the heat death of the universe.
