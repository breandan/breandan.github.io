...
If all we wanted was to exhaustively search some space, we could just enumerate items in lexicographic order and be done with it. But what if the positive samples were all concentrated at the end, or in the middle, or where we least expect them? We might have to wait a very long time to get the first answer. If we had no prior knowledge about where to find the solutions, where to begin? Any order we impose could be suboptimal without prior knowledge of the domain, so the best we can do is to sift through it uniformly. Best assume positive samples are uniformly distributed across the whole space.

At first glance, uniform sampling might not seem all that useful as there are more efficient strategies, but before we think about optimization, we must first *explore*. The key property we want from a uniform random sampler is that it explores all subspaces evenly and produces a steady stream of results -- not all clumped together in some parts and sparse in others. If there are clumpy parts containing an abnormal ratio of good to bad guesses, that's not what we want.

Our sampler should draw positive and negative samples with wall-clock frequency roughly proportional to their true density across the entire sample space. Another way of putting it is, we want the sequence of draws to be temporally invariant with respect to translation and scale: sufficiently long subsequences should all be equally representative of the full sequence. This property is known as *ergodicity*.
...
How do we ensure this property holds? We could enumerate every element, then scramble the search space using Fisher-Yates or Knuth shuffle. Again this might work for small sets, but would require storing the entire space in memory, so we're back to square one. Instead, what if we drew a random integer from a range equal to the cardinality of the sample space, and use the resulting integer to construct our guess directly?
...
There are a few subtleties here: first, we must be careful implementing `constructData()` - it should be an [injection](https://en.wikipedia.org/wiki/Injective_function) onto our datatype `<T>`. Since integers in most languages are limited to 2³², this might not be enough bits to represent the full space -- we might forget to map onto some elements, or over-represent others. Second, `Random.nextInt()` will eventually draw duplicates even when using a proper bijection. That might not be a big deal if the sample space is very large, but becomes problematic when sampling without replacement from small to medium-size sets. 
...
This solves the issue of duplicates and runs in nearly constant time at first, but consumes an increasing about of time and space as more samples are drawn. While there are probabilistic datastructures like Bloom filters to mitigate the spatial growth while retaining constant time membership queries, these do not address the issue of rejection sampling. We want a unified procedure for sampling without replacement from small, medium and large cardinality sets alike in constant time: we should be able to calculate how much wall clock time it will take to draw `K` samples after drawing `J` samples for `J << K`.
...
It turns out all of the above criteria are equisatisfiable. We will now describe an elegant procedure for doing so, using a different kind of algorithm.
...
So we can sample random elements of a single set, but how do we generalize this to higher dimensional sets? This is useful when we want to sample without replacement from a sequence of draws, such as a sequence of words in natural language which have some complex inter-dependence that requires modeling the full joint distribution. Let's take a closer look at the enumerative guesser we saw earlier. Suppose we want to generalize this sampling procedure to higher dimensional distributions.

Typically, when we have a sequence of words `T=t₁, t₂, ..., tₙ`, we might assume that the probability of `tᵢ` depends only on the previous thing, i.e., `P(T) = P(T₁) P(T₂|T₁) ... P(Tₙ | Tₙ₋₁)`. This is called the Markov assumption and if it holds, makes the distribution much more tractable. But what if we *really* want to model the full joint distribution? This often ruled out due to its asymptotic complexity, but as we will see, we can compute the joint distribution exactly or approximate for surprisingly large values of `n` with some efficient datastructures. 
...
Instead, we would prefer if each character in each index position is drawn randomly between consecutive samples. Remember the ergodicity criteria from earlier: we would like to generate a sequence of strings where each substring has equal probability between samples, whilst ensuring the entire string is sampled *without replacement*. How do we achieve this? Let's assume the alphabet is a power of two. Other bases are also possible, but powers of two are convenient for a variety of reasons.
...
So in order to generate quadruplets from the alphabet `Σ'`, we would need a bit string of length 16. Given a primitive polynomial on a bitvector of length 16, we can simulate draws from the space `Σ⁴`, since both are exactly equal in cardinality. How convenient!

So we can generate Boolean vectors. How do we map this back into vector spaces, whose cardinality is not necessarily an even power of two? One suggestion might be to use the leading bits, but that would break the injection, since the LFRS is only guaranteed to be aperiodic up to the full degree polynomial. Instead, we will encode the intermediate state into variable-length chunks:
...
The chunks will represent individual dimensions of the hyperdistribution. If each slot is drawn from the same sample space we simply assign equal-length bitvectors, requiring `d * ⌈log₂(c)⌉` bits where `d` is the number of dimensions and `c` is the cardinality of each. Otherwise, to index dimensions whose cardinality varies, we need a bitvector which can accommodate the concatenation. E.g., if the first dimension has cardinality `7`, we give it three bits, and the second has cardinality `3` we give it two bits and if the third dimension has cardinality `(2ⁿ+1, 2ⁿ⁺¹)` bits, we give it `n` bits, for a total of `3 + 2 + n + 1`. Then we just call `toIndexes(listOf(3, 2, n+1))` on a state to get a list of integers indexing into our coordinate sets.

You might have noticed that these sets are unlikely to have a power-of-two-sized cardinality. This layout problem is a recurring issue in array programming: it is seldom the case we can create an exact bijection with the Galois field. Oh no! To deal with this issue, cryptographers came up with something called the Hasty Pudding trick. No not [that Hasty Pudding 🍮](https://en.wikipedia.org/wiki/Hasty_pudding). [This hasty pudding 🔏](https://en.wikipedia.org/wiki/Hasty_Pudding_cipher).

Basically, we iterate the block cipher, throwing out any sample which contains a chunk exceeding the maximum cardinality in any indexed dimension until each contiguous subsequence of the bit vector is witnessed by a corresponding index in our coordinate set. In other words, we sample from the codomain `𝔹ⁿ`, then retain only those samples with a valid preimage. This operation will preserve the injection.
...
In the worst case, we will need to reject at most `~(1/2)^#dimensions` samples. This can be a problem in very high dimensions whose cardinalities are all far from powers of two, e.g. `(2ⁿ + 1)ᵈ where n, d->∞`, but even then, the cost of rejection will be dwarfed by the downstream costs of checking. There are higher radix Galois fields `GF(pᵏ)` for primes `p`, which are denser than `2ⁿ` and would improve sample complexity, however the underlying sampler is extremely fast already and even if we must reject thousands of samples, the cost of rejection will be negligible at modern clock frequencies.

Putting this whole thing together, we have two methods, one for coordinate systems like `Bⁿ` and one for things like `A×B×C` where `A, B, C` are dimensions of differing cardinality.
...
This is basically the same guess-and-check method we saw earlier with a twist - since we know the CDF is monotonically increasing function, we can speed up convergence via binary search. However, it requires 𝒪(n) space and time to compute the initial CDF, then 𝒪(log(n)) for each sample thereafter, which can be problematic if the sample space is very large. But we can do better! The Alias method is a constant-time weighted random sampler which precomputes in 𝒪(n·log(n)), then we jump to the right spot in 𝒪(1):
