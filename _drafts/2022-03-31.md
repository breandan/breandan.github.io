...
In probabilistic programming, is often necessary to draw samples from a space whose cardinality is much larger than the number of bits available to process it in memory. What if someone told you that no matter how large your sample space, you can draw samples in constant space and time, whilst never producing the same sample twice? This is indeed possible thanks to a beautiful theory developed by a young French mathematician named Évariste Galois in the early 19th century. Today we can benefit from his work via an extremely fast no-replacement uniform random sampler using cellular automata, Kotlin and some cool programming langauge theory. Let's get started!
...
How do we ensure this property holds? We could enumerate, then scramble the search space using Fisher-Yates or Knuth shuffle. But that would require storing the entire space in memory, so we're back to square one. Instead, what if we drew a random integer representing the bit pattern of the object and use that integer to construct our guess directly?
...
Uniform sampling requires unbiased random samples. But where does randomness come from? The best source of randomness is said to come from the decay of subatomic particles. However, if we need reproducibility, we must look elsewhere. The best alternative is something called a pseudorandom number generator (PRNG), which takes some input and scrambles it up somehow so as to be completely unrecognizable. A good PRNG will be indistinguishable from an RNG to any observer who does not know its internal state, and whose internal state cannot be easily inferred by observing the outputs -- however given the same state, it will produce the same values should the need for reproducibility later arise.

Deterministic or pseudo-random number generators (PRNGs) can be built using various techniques, all of which are essentially discrete dynamical systems known as cellular automata, Boolean dynamical systems, or pseudorandom number generators. Some of the best PRNGs naturally have the property of also being no-replacement samplers. Below is an example of one such PRNG:
...
- **Additive and multiplicative associativity**: Matched parentheses around additive and multiplicative sequences can be added wheresoever one pleases, i.e., `a*(b*c) = (a*b)*c`, `a+(b+c) = (a+b)+c`.
...
So we can draw random elements of a single set, but how do we generalize this to multiple dimensions? This is useful when we want to sample without replacement from a sequence of draws, like we do in natural langauge. Let's take a closer look at the enumerative guesser we saw earlier. Suppose we want to generalize this to higher dimensional distributions.
...
The problem here is that the bit patterns are fixed depth-first: if draw a bad choice at the beginning, it might take a very long time to back out of it. See what it generates:

```
L̂ = 0(0|1)*
     v-Bad initialization
    [1]00010 ∉ L̂
    [1]00011 ∉ L̂
    [1]00001 ∉ L̂
    [1]00000 ∉ L̂
...
Remember the ergodicity criteria from earlier: we would like to generate a sequence of strings where each character in each index position is drawn randomly between consecutive samples, whilst ensuring the entire string is sampled *without replacement*. How do we achieve this? Let's assume the thing is a power of two. Other bases are also possible, but power of two are convenient for hardware.
