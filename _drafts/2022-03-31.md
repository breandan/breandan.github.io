...
At first glance, uniform sampling might not seem all that useful as there are much more intelligent algorithms if the distribution is structured. But this is the magic of machine learning. We can choose any random projection and the algorithm still learns. The key property we want from a random sampler is that it covers the space and produces a steady stream of results, not all clumped together in some places and bad in others. This requires that wherever the data distribution lies, we intersect it somtimes. Later we can think about adaptive sampling, but first we must explore.

How do we sample a random order? We could use a Fisher-Yates shuffle. But that requires we store the entire space in memory, so we're back to square one. What if we drew a random integer representing the bit pattern of the object and construct it directly? Many useful things are enumerable. There are good space filling curves and Gray codes. It's worth a try.
...
To draw random samples without replacement, we need to understand where random numbers come from in the first place. The best random numbers come from nature. If we need truly irreversible random numbers, you'll want a quantum sensor. The problem with that is we also want it to be reproducible, i.e., deterministic. Deterministic or pseudo-random number generators (PRNGs) can be built using various techniques, all of which are essentially cellular automata. Below is a simple cellular automata, Boolean dynamical system, or pseudorandom number generator.
...
fun LFSR(degree: Int = 16): Sequence<UInt> = sequence {
  ...
  var vec = vec0
  val taps = generator[degree].mapIndexedNotNull { i, c -> if (c == '1') i else null }
  do {
...
fun LFSR(
    degree: Int,
    initialState: Vector = Vector(false, true, false, true, true),
    matrix: Matrix = Matrix.of(algebra, width = degree, height = degree) { r, c ->
        if (r == 1) generator[degree][c] == 0
        else if (c == r - 1) 1
        else 0
    }
): Sequence<UInt> = sequence {
    var m = matrix * initialState
    do {
        m = matrix * initialState
        yield(m.toUInt())
    } while (m != initial)
}
```
...
If we choose these coefficients `a`..`e`, carefully, the period will be full. This means the state vector will not repeat itself for 2^5 time steps, i.e., the cardinality of the underlying set.
...
This operation will preserve the injection. In the worst case, we will need to reject at most `~(1/2)^|cardinalities|` samples. This can be a problem in very high dimensions whose cardinalities are all far from powers of two, e.g. `(2^n + 1)^d where n, d->∞`, but since the underlying sampler is extremely fast, in practice even if we need to reject 1000s of samples, the cost of rejection will be dwarfed by downstream costs of checking.
