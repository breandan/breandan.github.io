...
A common problem in computer science is to generate a set of feasible solutions to a search problem, i.e., solutions meeting a set of criteria. Occasionally these solutions can be constructed directly using a clever algorithm, but in many cases, either no algorithm exists or is known for finding the solutions directly. In such cases, we must resort to a search procedure that might be described as "guess and check". Let's just focus on the "guessing" part. Pursuing this idea seriously leads to some surprisingly intricate theory.
...
In this procedure all guesses are first generated to an intermediate list, then we iterate through all items in the list, filtering by the criteria, and only then do we return the results. There are a few problems here: first the search space might not fit it in memory! Second, even if it did, filtering it before returning a single answer could take more time than we have to spare. Finally, depending on how `guesser` and `checker` were implemented, it might generate repeats.
...
In common parlance, "I don't know" can be seen as an admission of ignorance -- some might even call this a moral failing. On the contrary, in probabilistic programming, we call this a virtue! Serious dedication to this philosophy may even be called wisdom. The best guessers are utterly uninformed.

While we could just enumerate items in lexicographic order if our goal was exhaustive search, what if the positive samples are all concentrated at the end, or in the middle, or in odd spots? We might have to wait a very long time to get the first answer. If we had no prior knowledge about where the solutions lie, where do we begin? Any artificial order we impose could be suboptimal without prior knowledge of the domain, so the best we can do is to sift through it uniformly. Best we assume positive samples are equidistributed across the whole space.

At first glance, uniform sampling might not seem all that useful as there can be more efficient sampling strategies, but before we think about adaptive sampling, we must first *explore*. The key property we want from a random sampler is that it explores all subspaces evenly and produces a steady stream of results -- not all clumped together in some parts and sparse in others. If there are clumpy parts containing an abnormal ratio of good to bad guesses, that's not what we want.

Our sampler should draw positive and negative samples with wall-clock frequency roughly proportional to their true density across the entire sample space. Another way of putting it is, we want the sequence to be temporally invariant with respect to translation and scale: sufficiently long subsequences should all be equally representative of the full sequence. This property is known as *ergodicity*.
...
How do we ensure this property holds? We could enumerate, then scramble the search space using Fisher-Yates or Knuth shuffle. But that would require storing the entire space in memory, so we're back to square one. Instead, what if we drew a random integer from a range equal to the cardinality of the sample space, and use the resulting guess to construct our guess directly?
...
There are a few issues here: first, we must be careful implementing `constructData()` - it should be a [bijection](https://en.wikipedia.org/wiki/Bijection). Since integers in most languages are limited to 2^32, this might not be enough bits to represent the full space -- we might forget to map onto some elements, or over-represent others. Second, `Random.nextInt()` will eventually draw duplicates even when using a proper bijection. That might not be a big deal if the sample space is very large, but becomes problematic when sampling without replacement from smaller sets. Suppose we have a set too large to fit in memory but still tractable to search through. We could store a table of visited items and filter for unseen samples:
...
This solves the issue of duplicates and runs in nearly constant time at first, but consumes an increasing about of time and space as more samples are drawn. While there are probabilistic datastructures like Bloom filters which mitigate the spatial growth and offer constant time membership queries on large-cardinality sets, these do not address the issue of rejection sampling. We want a unified procedure for sampling without replacement from small, medium and large cardinality sets alike in constant time: we should be able to calculate how much wall clock time it will take to draw `K<∞` samples after drawing `J` samples for `J << K`.
...
Uniform sampling requires unbiased random samples. But where does randomness come from? The best source of randomness is said to come from decaying subatomic particles. However, if we need reproducibility, we must look elsewhere. The best alternative is something called a pseudorandom number generator (PRNG), which takes an input and scrambles it up somehow to be completely unrecognizable. A good PRNG will be indistinguishable from an RNG to any observer who does not know its internal state, and whose internal state cannot be easily inferred by observing the outputs -- however given the same state, it will produce the same values should the need for reproducibility later arise.
...
The astute reader might notice that since we are using a matrix, the time and space complexity is not constant. However, we note that the matrix is sparse, computing the matrix-vector product can be reduced to exactly six bitwise operations. It can be visualized as a digital circuit:
...
These structures have lots of applications in number theory, computer science, cryptography, and as it turns out, probabilistic programming. We can describe Galois fields of degree `2^n` using a special kind of function that maps the field back onto itself called a feedback polynomial. A subset of these, called primitive polynomials, have the envious property of being minimal length and maximal periodicity. These two properties make it highly desirable: a primitive polynomial on `GF(2^n)` can generate the entire field using `~O(log(n))` bits.

This means the primitive polynomial is a space-filling curve, i.e., a curve which visits all `2^n-1` elements of the set exactly once before repeating. Furthermore, it does so in pseudorandom order: unlike simple enumerative procedures like Hilbert Curves or Gray Codes, the PP takes a nontrivial trajectory through its state space making difficult to predict without knowing the polynomial a priori, although not impossible. More sophisticated versions exist which increase the difficulty of inversion if secrecy is important.
...
How do we find primitive polynomials? We could simply enumerate all degree-S polynomials up to `log_p(|S|)` and check whether they are full period. It so happens we have already seen a guess-and-check algorithm that can generate examples in random order. This search procedure works for low-order polynomials but grows intractable with increasing degree.

The beauty of primitive polynomials over GF(2) is that they can be composed using other primitive polynomials. This fact allows us to write a decision procedure which eliminates certain guesses using a static property of the polynomial, rather than measuring its periodicity by simulating the dynamics, which can take until the heat death of the universe.
...
<div class="quote"><blockquote><b>Preface to “Two Memoirs on Pure Analysis”</b> – Evariste Galois, A.D. 1811-1832<br>
...
So we can sample random elements of a single set, but how do we generalize this to higher dimensional sets? This is useful when we want to sample without replacement from a sequence of draws, such as a sequence of words in natural language which have some interdependence. Let's take a closer look at the enumerative guesser we saw earlier. Suppose we want to generalize this sampling procedure to higher dimensional distributions.
...
fun all(i: List<Int>, l: List<Int> = emptyList()): Sequence<List<Int>> =
    if (i.isEmpty()) sequenceOf(l)
        ...
        .shuffled().flatMap { all(i.drop(1), l + it) }
```

The problem here is that the bit patterns are fixed depth-first. Notice that if we draw an unlucky prefix, it might take a very long time to back out of that assignment:
...
Instead, we would prefer if each character in each index position is drawn randomly between consecutive samples. Remember the ergodicity criteria from earlier: we would like to generate a sequence of strings where each substring has equal probability between samples, whilst ensuring the entire string is sampled *without replacement*. How do we achieve this? Let's assume the string is a power of two. Other bases are also possible, but powers of two are convenient for a variety of reasons.
...
So in order to generate quadruplets from the alphabet `Σ`, we would need a bit string of length 16. And given a primitive polynomial on a bitvector of length 16, we can simulate draws from the space `Σ^4`, since both are exactly the same size. How convenient!

Okay, so we can generate Boolean vectors. How do we map this back into vector spaces, whose cardinality might not necessarily be a power of two? One way might be to take the leading bits, but that would destroy the involution, since the LFRS is only guaranteed to be unique up to the full degree polynomial. Instead, we will encode the intermediate state into variable-length chunks:
...
The chunks will represent individual dimensions of the hyperdistribution. So if the first dimension has cardinality `7`, we give it three bits, and the second has cardinality `3` we give it two bits and if the third dimension has cardinality `(2^n-1, 2^(n+1))` bits, we give it `n` bits. Then we can call it with a state to get a list of integers, indexing into our coordinate sets -- this will allow us to index dimensions whose cardinality varies. If each slot is drawn from the same sample space, each dimension will be the same size, so this will only require `d * ⌈log(c)⌉` bits where `d` is the number of dimensions and `c` is the cardinality of each dimension.

You might have noticed that these sets are unlikely to have cardinality that is exactly a power of two. Thus, we cannot create a bijection with the Galois field. Oh no! But fear not, we can use something called the Hasty Pudding trick. No not that [Hasty Pudding](https://en.wikipedia.org/wiki/Hasty_pudding). This hasty pudding:

https://en.wikipedia.org/wiki/Hasty_Pudding_cipher

Basically, we iterate the block cipher, throwing out any sample which contains a chunk exceeding the maximum cardinality in any indexed dimension until each contiguous subsequence of the bit vector is witnessed by a corresponding index in our coordinate set. This is equivalent to sampling from the codomain `𝔹ⁿ`, then retaining only those with a valid preimage. This operation will preserve the injection.
...
In the worst case, we will need to reject at most `~(1/2)^#dimensions` samples. This can be a problem in very high dimensions whose cardinalities are all far from powers of two, e.g. `(2^n + 1)^d where n, d->∞`, but in almost all cases, the cost of rejection will be dwarfed by the downstream costs of checking. There are higher radix Galois fields `GF(p^k)` for primes `p`, and since the gaps between consecutive primes are much smaller than `2^n` this would improve sample complexity. But as the underlying sampler is extremely fast already and all digital hardware is binary-valued, even if we need to reject thousands of samples, the cost of rejection will be negligible at modern clock frequencies.
...
How do we adapt this to sampling without replacement? Here we need to precompute alias tables for each element of the powerset. Once we do so, we can draw weighted random samples without replacement in 𝒪(1) time and 𝒪(2^n) space. This is only really practical for small sets. Maybe we could do it dynamically for larger sets and cache the results.
