...
Automated criminality inference based on facial images is a [real thing](https://arxiv.org/abs/1611.04135), and [not just in China](https://www.propublica.org/article/machine-bias-risk-assessments-in-criminal-sentencing). Although many flaws exist in these systems, the underlying assumptions are probably correct. With this kind of technology, if it works, somebody will find a way to use it. But even if AIs could explain their logic, the problem is not the algorithms themselves. The problem is those who are willing to apply them indiscriminately, regardless of whether they understand the rules. If some algorithm is 45% confident you're going to default on a home loan, the bank isn't going to debug their lending bot until it starts loosing money. The incentives are not aligned in your favor.
...
As [experience](http://www.theatlantic.com/technology/archive/2016/04/the-underlying-bias-of-facial-recognition-systems/476991/) has [shown](https://www.theguardian.com/technology/2016/sep/08/artificial-intelligence-beauty-contest-doesnt-like-black-people), anticipating mistakes when designing AI is difficult enough, never mind verifying its integrity as a subject - if you know the AI exists begin with. But let's say you have prior knowledge, motive, and a deep background in statistics. If there is an open API and the designer is not careful, you might be able to [steal the model](https://www.usenix.org/system/files/conference/usenixsecurity16/sec16_paper_tramer.pdf), and check it yourself. That's a lot of "if"s. The average end-user has little hope of ever verifying the fairness of an AI, and no motive unless they are unfairly targeted. If you are unfairly targeted by an AI, the incentives are not in your favor.

Let's suppose you are unfairly targeted by an AI. The lawsuit-worthy cases are going to be tested prior to release, and the easy-lawsuits will be fixed early, to avoid any appearance of discrimination. If you suspect yourself to be the victim of unfair discrimination in an AI decision process, you will face a series of uphill battles. First you will need to prove the AI  exists. Then you will need to prove the AI had a significant influence in making the decision. Then you will need to prove a statistically significant number of samples where prior discrimination occurred. Good luck getting a subpoena for terabytes of [PII](https://en.wikipedia.org/wiki/Personally_identifiable_information).

> Trust, but verify.
>
> -[Some Russian](https://en.wikipedia.org/wiki/Trust,_but_verify)

The problem is not the algorithms. The problem is the human beings implementing them. The people training self-driving cars are [college dropouts](https://www.bloomberg.com/features/2015-george-hotz-self-driving-car/) and ordinary software developers with something called a "[nanodegree](https://www.udacity.com/nanodegree)" in machine learning. If you're lucky, maybe some of them have a degree in statistics. Knowing developers, if there is a catastrophic failure, it will more likely be out of [stupidity than malice](https://en.wikipedia.org/wiki/Hanlon's_razor). There needs to be be some kind of regulation for AI that can literally kill you. Or at the very least, a board certification for data scientists, like doctors and lawyers. This is highly unlikely ever to happen.

While we're on the subject of data science, let's talk a little about bias. "Bias" in the statistical sense is not a bad word. Statisticians have grown so accustomed to the it because bias is literally everywhere. It is fundamentally how we make a decision in the first place. If you are designing AI in a new domain, you should ask yourself three important questions:
