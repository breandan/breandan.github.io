...
The IntelliJ Platform officially supports Linux operating systems running GNOME or KDE. The following script installs a JetBrains IDE on any [Debian based Linux distro](https://en.wikipedia.org/wiki/List_of_Linux_distributions#Debian-based). First paste `wget `[`z.ndan.co/jetbrains-install.sh`](z.ndan.co/jetbrains-install.sh) into a terminal to fetch the script below:
